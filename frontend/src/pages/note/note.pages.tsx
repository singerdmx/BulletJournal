...
import {DeleteTwoTone, PlusCircleTwoTone, TagOutlined, UpSquareOutlined,} from '@ant-design/icons';
// modals import
...
const NotePage: React.FC<NotePageHandler & NoteProps> = (props) => {
    ...
    const createContentElem = (<Button onClick={createHandler}>
        <PlusCircleTwoTone/>
        New
    </Button>);

    const noteEditorElem = (<div className='note-drawer'>
        <NoteEditorDrawer
            noteId={note.id}
            visible={showEditor}
    ...
    const noteOperation = () => {
        return <div className='note-operation'>
    ...
    return (<NoteDetailPage note={note} labelEditable={labelEditable} noteOperation={noteOperation}
                            createContentElem={createContentElem} noteEditorElem={noteEditorElem}/>);
};
