...
function* updateRecentDates(action: PayloadAction<DatesReceivedAction>) {
    try {
        ...
        data = yield call(fetchRecentProjectItems, types, timezone, startDate, endDate);
        yield put(recentActions.recentItemsReceived({items: data}));
        yield put(recentActions.datesReceived({startDate: startDate, endDate: endDate}));
    } catch (error) {
...
export default function* recentSagas() {
    yield all([
        yield takeLatest(
            recentActions.updateRecentDates.type,
            updateRecentDates
        ),
    ]);
}